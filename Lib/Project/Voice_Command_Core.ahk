...
;============================================================
; MODE SWITCHING - SAPI / VOSK
;============================================================

/** @description SwitchToVosk - Pause SAPI grammar and hand mic to Vosk bridge */
SwitchToVosk() {
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . 'Started', 1)
    global objGrammar, objControlGrammar, blnListening, strVoiceMode

    if strVoiceMode = "vosk" {
        ToolTip("Already in Vosk mode")
        SetTimer(() => ToolTip(), -1500)
        return
    }

    ; Pause both SAPI grammars so SAPI releases the mic
    try {
        objGrammar.CmdSetRuleState("cmd", 0)
    ...
    ToolTip("Vosk: sentence recognition ON")
    SetTimer(() => ToolTip(), -2000)
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . "Switched to Vosk mode", 2)
}

/** @description SwitchToSapi - Release mic from Vosk and resume SAPI grammar */
SwitchToSapi() {
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . 'Started', 1)
    global objGrammar, objControlGrammar, blnListening, blnCommandsEnabled, strVoiceMode

    if strVoiceMode = "sapi" {
        ToolTip("Already in SAPI mode")
        SetTimer(() => ToolTip(), -1500)
        return
    }
    ...
    ToolTip("SAPI: command mode ON")
    SetTimer(() => ToolTip(), -2000)
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . "Switched to SAPI mode", 2)
}

/** @description ToggleModeVoskSapi - Toggle between SAPI and Vosk modes (F3) */
ToggleModeVoskSapi() {
    global strVoiceMode
    if strVoiceMode = "vosk"
        SwitchToSapi()
...
/** @description ToggleSpeakLanguage - Toggle Vosk language between default and special (F4) */
ToggleSpeakLanguage() {
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . 'Started', 1)
    global strSpeakLanguage, strBridgeLanguage, strIniFile

    if strBridgeLanguage = "" {
        ToolTip("No special language configured (add Language= to INI)")
        SetTimer(() => ToolTip(), -3000)
        return
    }

    if strSpeakLanguage = "default" {
        strSpeakLanguage := "special"
        BridgeSend("LANG:special")
        ToolTip("Language: " strBridgeLanguage " (special)")
    } else {
        strSpeakLanguage := "default"
        BridgeSend("LANG:default")
        ToolTip("Language: English (default)")
    }

    IniWrite(strSpeakLanguage, strIniFile, "Settings", "SpeakLanguage")
    SetTimer(() => ToolTip(), -2000)
    LogMsg(FFL(A_ThisFunc, A_LineNumber) . "Language toggled to: " strSpeakLanguage, 2)
}
